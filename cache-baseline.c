	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (cp->hsize)
    {
      ...
      for (blk=cp->sets[h0].hash[hindex];
	   blk;
	   blk=blk->hash_next)
	{
  ...
  case LRU:
  case FIFO:
    repl = cp->sets[h0].way_tail;
    update_way_list(&cp->sets[h0], repl, Head);
    break;
  case Random:
    {
      int bindex = myrand() & (cp->assoc - 1);
      repl = CACHE_BINDEX(cp, cp->sets[h0].blks, bindex);
    }
    break;
  default:
  ...
  if (blk->way_prev && cp->policy == LRU)
    {
      ...
      update_way_list(&cp->sets[h0], blk, Head);
    }
