...
/*#define CACHE_HASH1(cp, key)					\
  (((key >> 4) ^ (key >> 2) ^ (key << 1) ^ key) % cp->nsets)
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
	...
	h2=CACHE_HASH3(cp, i);
	h3=CACHE_HASH4(cp, i);
	cp->sets[h0].way_head = NULL;
	cp->sets[h3].way_tail = NULL;
	cp->sets[h0].way_one = NULL;
	cp->sets[h1].way_two = NULL;
	cp->sets[h2].way_three = NULL;
	cp->sets[h3].way_four = NULL;
      }
      else {
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (strcmp(cp->name, "ul2") != 0){  
   ...
   /*
    ...
    if (blk && (blk->tag == tag && (blk->status & CACHE_BLK_VALID)))
      goto cache_hit;
